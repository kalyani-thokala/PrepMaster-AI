...
const callWithRetry = async (fn, retries = 2) => {
  ...
  logger.error("❌ Gemini failed after retries:", lastError);
  return null;
};
...
const callGeminiText = async (
  ...
  if (!client) {
    return "AI is currently in fallback mode. Please configure GEMINI_API_KEY.";
  }
...
"""`;
    ...
    const system =
      "You are an ATS resume expert and HR recruiter. Return structured JSON.";
  ...
  ========================= */
  evaluateMockInterview: async (role, questionText, answerText) => {
    if (!role || !questionText || !answerText) return null;
...
Return JSON with score and feedback.`;
    ...
    const system =
      "You are a senior technical interviewer. Be strict and structured.";
    ...
    return {
      score: 70,
      technicalAccuracy: "Basic understanding",
      communication: "Average",
      confidence: "Moderate",
      clarity: "Needs structure",
      feedbackText: "Improve structure and add examples.",
  ...
  generateAssessment: async (category, difficulty) => {
    ...
    const prompt = `Generate 5 MCQs for ${category} (${difficulty}) in JSON format.`;

    const system =
      "You are an exam question generator. Always return valid JSON.";
  ...
  generateCareerCoachResponse: async (userMessage, chatHistory = []) => {
    ...
    const system =
      "You are a career mentor helping students get jobs.";

    return await callGeminiText(userMessage, system, chatHistory);
  },
  ...
  generateRoadmap: async (targetRole, currentSkills = []) => {
    ...
    const prompt = `Create roadmap for ${targetRole}. Skills: ${currentSkills.join(
      ", "
    )}`;

    const system =
      "You are a curriculum designer. Return structured JSON.";
  ...
  ========================= */
  evaluatePlacementReadiness: async (
    fullName,
    skills = [],
    resumeStats = {},
    assessmentStats = {},
    ...
    const prompt = `Evaluate placement readiness for ${fullName}.`;

    const system =
      "You are a recruitment director evaluating students.";
    ...
    return {
      overallReadiness: 65,
      codingReadiness: 60,
      aptitudeReadiness: 70,
      communicationReadiness: 75,
      interviewReadiness: 55,
      recommendedActions: ["Practice more coding problems"],
    };
  },
  ...
  ========================= */
  generateAptitudeQuestions: async (
    topic,
    difficulty,
    count,
    excludeQuestions = []
    ...
    const prompt = `Generate ${count} MCQs for ${topic} (${difficulty}).`;
  ...
  generateCodingChallenge: async (topic, difficulty) => {
    ...
    const prompt = `Generate coding challenge for ${topic} (${difficulty}).`;
  ...
  evaluateCodeSubmission: async (
    ...
    const prompt = `Evaluate code quality for ${title}.`;

    const system =
      "You are a senior software engineer. Return JSON only.";
    ...
    const ratio =
      testCasesResults.passedCases /
      (testCasesResults.totalCases || 1);

    return {
      score: Math.round(ratio * 75),
      feedback: "Code evaluated with fallback mode",
      timeComplexity: "O(N)",
      spaceComplexity: "O(N)",
  ...
  generateRecommendations: async (weakTopics = []) => {
    ...
    const prompt = `Weak topics: ${weakTopics.join(", ")}`;

    const system = "Return structured JSON only.";

    const result = await callGeminiJson(prompt, system);

    if (result?.recommendedTopics) return result;

    return {
      guidance: "Focus on fundamentals",
      recommendedTopics: weakTopics.map((t) => ({
        topic: t,
